...
def main(args: Optional[list[str]] = None) -> None:
    ...
    df_hist.plot(x="step", y="loss", ylim=(0, 1) if min_loss < 0.5 else None, ax=ax2, legend=False)  # type: ignore[arg-type]
    ax2.set_xlabel("Iteration")
    ax2.set_ylabel("Loss")

    df_eval.plot(x="epoch", y="accuracy", ylim=(0, 100), ax=ax3, legend=False)
    ...
    ax3.text(x=max_epoch, y=5, s=f"Max: {max_acc:.2f}% @ {max_epoch} ", ha="right", va="bottom")
    ax3.plot([0], [max_acc])

    if config.output == "none":
        plt.show()
    else:
        file_path = os.path.join(output_dir, f"fig_history.{config.output}")
