...
def untuned_exponential_period(beta2: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    return 1.0 / (np.exp(1.0 - beta2) - 1.0)


def untuned_linear_period(beta2: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    return 1.0 / (1.0 - beta2) - 0.5


def warmup_factor(
    steps: npt.NDArray[int], beta2: float, rho_inf: float, offset: int
) -> npt.NDArray[np.float64]:
    rho = rho_fn(steps + offset, beta2, rho_inf)
    numerator = (rho - 4) * (rho - 2) * rho_inf
    denominator = (rho_inf - 4) * (rho_inf - 2) * rho
    return np.sqrt(numerator / denominator)


def radam_period_fn(beta2: np.float64) -> np.float64:
    rho_inf = rho_inf_fn(beta2)
    offset = get_offset(beta2, rho_inf)
    steps = np.arange(1, 101)
    w = warmup_factor(steps, beta2, rho_inf, offset)
    total_sum: np.float64 = np.sum(1 - w)
    t = 1
    while True:
        steps = np.arange(100 * t + 1, 100 * (t + 1) + 1)
        w = warmup_factor(steps, beta2, rho_inf, offset)
        partial_sum: np.float64 = np.sum(1 - w)
        if partial_sum < 0.1:
            break
...
def radam_period(beta2: npt.NDArray[np.float64]) -> list[np.float64]:
